...
app.post('/chat', async (req, res) => {
  ...
  try {
    // Limita dados para não exceder o payload
    const dadosResumidos = JSON.parse(JSON.stringify(dados));
    if (dadosResumidos.pedidos) dadosResumidos.pedidos = dadosResumidos.pedidos.slice(0, 20);
    if (dadosResumidos.produtos) dadosResumidos.produtos = dadosResumidos.produtos.slice(0, 30);
    if (dadosResumidos.estoque) dadosResumidos.estoque = dadosResumidos.estoque.slice(0, 30);
    if (dadosResumidos.receber) dadosResumidos.receber = dadosResumidos.receber.slice(0, 15);
    if (dadosResumidos.pagar) dadosResumidos.pagar = dadosResumidos.pagar.slice(0, 15);
    if (dadosResumidos.nfe) dadosResumidos.nfe = dadosResumidos.nfe.slice(0, 10);
